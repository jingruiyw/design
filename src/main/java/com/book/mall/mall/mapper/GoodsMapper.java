...
public interface GoodsMapper {
    ...
    @Insert("insert into goods(name, price, number, kind, image, status, remark,create_time) values(#{name}, #{price},#{number},#{kind},#{image},#{status},#{remark},#{createTime})")
    public int addGoods(GoodsAddReqForm reqForm);

    @Update("update goods set name= #{name}, price= #{price}, number= #{number}, kind= #{kind}, image=#{image}, status=#{status}, remark=#{remark}, create_time=#{createTime} where id = #{id}")
