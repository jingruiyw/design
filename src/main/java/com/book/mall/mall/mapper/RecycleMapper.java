...
public interface RecycleMapper {
    ...
    @Insert("insert into recycle (name, number, open_id, kind_id, kind_name, seller, mobile_no, address, status, create_time) values (#{name}, #{number}, #{openId}, #{kindId}, #{kindName}, #{seller}, #{mobileNo}, #{address}, #{status}, #{createTime})")
